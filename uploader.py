from map_browser.models import Map, Archive, People, Languages, PublicationPlaces, SubjectTypes
import openpyxl
...
def get_fn_ln(instance, row, fn_col, ln_col):
    if row[fn_col].value is not None or row[ln_col].value is not None:
        ...
        if instance == 'user':
            obj, created = User.objects.get_or_create(username=fn+ln, password='Testing4321', first_name=fn, last_name=ln)
        elif instance == 'author':
            obj, created = People.objects.get_or_create(first_name=fn, last_name=ln)
        elif instance == 'place':
            obj, created = PublicationPlaces.objects.get_or_create(country_name=fn, city_name=ln)
        else:
            obj = None
        return obj
...
def get_archive(row, starting_col):
    ...
    if nazwa_arch is not None or nazwa_zespolu is not None or jednostka_arch is not None or nr_karty is not None:
...
def upload_excel_to_postgres():
    ...
    for row in sheet.iter_rows(2, 53):
        ...
        # creating map object and adding all values despite many to many
        current_map, created = Map.objects.get_or_create(
            filename=filename, thumbnail=thumbnail, archive_id=archive, corrector_id=corrector, full_title=full_title,
            short_title=short_title, publishing_address=pub_address, publication_place=pub_place,
            created_at=creation_year, scale=scale, subject=subject, creation_type=creation_type,
            description=description, source_text=source_txt, keyword_subject=keyword_subject,
            keyword_name=keyword_name, keyword_geo=keyword_geo, additional_notes=additional_notes
        )

        current_map.save()

        # Adding Map Creators and Authors
        creators = [get_fn_ln('user', row, 2, 3), get_fn_ln('user', row, 4, 5), get_fn_ln('user', row, 6, 7)]
        authors = [get_fn_ln('author', row, 11, 12), get_fn_ln('author', row, 13, 14), get_fn_ln('author', row, 15, 16)]
        ...
        lang = [x.strip().capitalize() for x in [row[29].value, row[30].value, row[31].value] if x is not None]
        lang_obj = []
        for single in lang:
        ...
        subject_type = [x.strip().capitalize() for x in [row[26].value, row[27].value, row[28].value] if x is not None]
        subject_type_obj = []
